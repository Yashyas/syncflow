...
export default function ChatWindow({ senderRole = Sender.freelancer }: ChatWindowProps) {
  ...
  useEffect(() => {
    ...
    channel.bind("new-message", (incomingMessage: Message) => {
      const current =
        useDashboardStore.getState().taskChats[taskId]?.messages ?? [];
      ...
      if (optimistic) {
        ...
        useDashboardStore.getState().appendMessage(taskId, incomingMessage);
      }
    });
    return () => {
