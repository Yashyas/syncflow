import { Message, Project, Task} from "@prisma/client";
...
interface PrismaTask extends Task{
  _count?:{
    messages:number;
  }
}
...
interface DashboardStore {
  ...
  tasks: PrismaTask[];
  setTasks: (tasks: PrismaTask[]) => void;
  addTask: (task: PrismaTask) => void;
  updateTask: (task: PrismaTask) => void;
...
export const useDashboardStore = create<DashboardStore>((set) => ({
  ...
  toggleProjectSelectionDrawer: () =>
    set((state) => ({
      isProjectSelectionDrawerOpen: !state.isProjectSelectionDrawerOpen,
    })),
  ...
  toggleAddProjectDrawer: () =>
    set((state) => ({ isAddProjectDrawerOpen: !state.isAddProjectDrawerOpen })),

  // 4. Delete project drawer toggle
  isDeleteProjectDrawerOpen: false,
  toggleDeleteProjectDrawer: () =>
    set((state) => ({
      isDeleteProjectDrawerOpen: !state.isDeleteProjectDrawerOpen,
    })),

  // 5.Share project drawer toggle
  isShareProjectDrawerOpen: false,
  toggleShareProjectDrawer: () =>
    set((state) => ({
      isShareProjectDrawerOpen: !state.isShareProjectDrawerOpen,
    })),

  // 6. Add task drawer toggle
  isAddTaskDrawerOpen: false,
  toggleAddTaskDrawer: () =>
    set((state) => ({ isAddTaskDrawerOpen: !state.isAddTaskDrawerOpen })),
  ...
  toggleUpdateTaskDrawer: () =>
    set((state) => ({ isUpdateTaskDrawerOpen: !state.isUpdateTaskDrawerOpen })),
  ...
  toggleDeleteTaskDrawer: () =>
    set((state) => ({ isDeleteTaskDrawerOpen: !state.isDeleteTaskDrawerOpen })),

  //11.Current dashboard view
  currentDashboardView: CurrentView.KANBAN,
  setCurrentDashboardView: (view: CurrentView) =>
...
// central chat 
export const useCentralChatTask = () =>
  useDashboardStore((state) =>
    state.tasks.find((t) => t.status === "central_chat") ?? null
  );